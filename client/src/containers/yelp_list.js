...
class YelpList extends Component {
  ...
  componentDidMount() { // fetches data from yelp open api
    const term = this.props.user[0] !== undefined ? this.props.user[0].lastsearch : 'orlando'
  ...
  updateWhosGoing(index, businessID) {
      return fetch(`https://nightlife-ps-serv.herokuapp.com/api/businesses?id=${businessID}`, {
        headers: {
  ...
  render() {
    console.log(this.state.whosGoing)
